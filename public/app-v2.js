...
function activateDashboardView(targetId, updateHash = true) {
  const allowedViews = new Set(["nationalModule", "priceParams", "provinceModule"]);
  const nextView = allowedViews.has(targetId) ? targetId : "nationalModule";
  ...
  if (document.body.classList.contains("dashboard-v2")) {
    els.navLinks.forEach((link) => {
      link.addEventListener("click", (event) => {
        ...
        activateDashboardView(link.getAttribute("href").slice(1));
      });
    });
    return;
  }
