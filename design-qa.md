...
11. Simplified province map tooltips by removing the repeated period and showing settlement reference prices only for an exact single-month selection with a maintained value. Verified province-name normalization, exact-month numeric/text references, and omission for multi-month or empty-month selections.
