...
def apply_historical_volume_mapping(info: dict[str, Any], province: str) -> None:
    if province in {"蒙东", "吉林"}:
        ...
        info["volumeColumns"] = {
            "日前": "日前文件：新能源负荷（周_光伏出力）_日前；为空时复用实时量",
            "实时": "实时文件：新能源负荷（周_光伏出力）_实时；为空时复用日前量",
        }
        info["volumeSource"] = {
            "日前": "优先日前光伏边界量，其次按日期时刻复用实时量；两边均无量时使用典型曲线",
            "实时": "优先实时光伏边界量，其次按日期时刻复用日前量；两边均无量时使用典型曲线",
        }
    info["usesTypicalCurve"] = province in {"云南", "广西", "贵州", "海南"}
